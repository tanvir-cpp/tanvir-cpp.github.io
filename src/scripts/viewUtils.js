...
export const showItem = (item, elements, options = {}) => {
  const { showThumbnail = false, stripFirstHeading = false } = options;
...
`;
  ...
  if (typeof marked === "undefined") {
    elements.markdownContainerResource.innerHTML =
      headerHTML + `<div class="max-w-3xl mx-auto">${item.content}</div>`;
  } else {
  ...
  if (typeof hljs !== "undefined") {
    elements.markdownContainerResource
...
export const hideItem = (elements) => {
  if (elements.resourceContent)
    elements.resourceContent.classList.add("hidden");

  if (elements.listingView) {
    elements.listingView.classList.remove("hidden");
  } else {
    if (elements.listingHeader)
      elements.listingHeader.classList.remove("hidden");
